...
type ProjectQuery interface {
	...
	GetPublicProjects(ctx context.Context) ([]*model.Project, error)
}
...
func (q projectQuery) GetProject(ctx context.Context, id string) (*model.Project, error) {
	...
	userReq := idStr // Assign the string value to userReq

	// Fetch the project using the given ID
	proj, err := q.projSrv.GetProject(id)
	if err != nil {
		return nil, err
	}
	...
	// If the project is not public, check if the requesting user is the owner
	if userReq != proj.OwnerId {
		return nil, fmt.Errorf("unauthorized access")
	}
...
func (q projectQuery) GetPublicProjects(ctx context.Context) ([]*model.Project, error) {
	projects, err := q.projSrv.GetPublicSharedProjects()
	if err != nil {
		return nil, err
	}
